...
// Fonction pour échapper les caractères spéciaux pour les expressions régulières
...
Object.keys(toolsTranslations).forEach(lang => {
  ...
  const regex = new RegExp(`(const ${escapeRegExp(lang)} = \{[\s\S]*?)(tools: \{[\s\S]*?\})([\s\S]*?// Technologies)`, 'g');
  
  // Remplacer la section tools
  content = content.replace(regex, `$1${toolsSection}$3`);
});
